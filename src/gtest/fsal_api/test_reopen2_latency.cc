...
TEST_F(Reopen2EmptyLatencyTest, SIMPLE)
{
	...
	status = test_file->obj_ops->reopen2(test_file, test_file_state,
					     FSAL_O_READ);
	ASSERT_EQ(status.major, 0);
}
...
TEST_F(Reopen2EmptyLatencyTest, SIMPLE_BYPASS)
{
	...
	status = sub_hdl->obj_ops->reopen2(sub_hdl, test_file_state,
					   FSAL_O_READ);
	ASSERT_EQ(status.major, 0);
}
...
TEST_F(Reopen2EmptyLatencyTest, FSAL_REOPEN2)
{
	...
	for (int i = 0; i < LOOP_COUNT; ++i) {
		if (i % 2 == 0) {
			status = fsal_reopen2(test_file, test_file_state,
					      FSAL_O_READ, false);
			ASSERT_EQ(status.major, 0);
		} else {
			status = fsal_reopen2(test_file, test_file_state,
